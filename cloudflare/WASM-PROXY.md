...
LocalPDF uses a Cloudflare Worker to proxy WASM library requests, bypassing CORS restrictions when loading AGPL-licensed components (PyMuPDF, Ghostscript, CoherentPDF) from external sources.
...
# Go to Workers & Pages > localpdf-wasm-proxy > Settings > Variables
```
...
> **Note:** You can use your own hosted WASM files instead of the recommended URLs. Just ensure your files match the expected directory structure and file names that LocalPDF expects for each module.

### 3. Configure LocalPDF
...
```bash
VITE_WASM_PYMUPDF_URL=https://localpdf-wasm-proxy.<your-subdomain>.workers.dev/pymupdf/
VITE_WASM_GS_URL=https://localpdf-wasm-proxy.<your-subdomain>.workers.dev/gs/
VITE_WASM_CPDF_URL=https://localpdf-wasm-proxy.<your-subdomain>.workers.dev/cpdf/
```

**Option B: Manual per-user configuration**

In LocalPDF's Advanced Settings (wasm-settings.html), enter:

| Module      | URL                                                                 |
| ----------- | ------------------------------------------------------------------- |
| PyMuPDF     | `https://localpdf-wasm-proxy.<your-subdomain>.workers.dev/pymupdf/` |
| Ghostscript | `https://localpdf-wasm-proxy.<your-subdomain>.workers.dev/gs/`      |
| CoherentPDF | `https://localpdf-wasm-proxy.<your-subdomain>.workers.dev/cpdf/`    |

## Custom Domain (Optional)

To use a custom domain like `wasm.localpdf.org`:

1. Add route in `wasm-wrangler.toml`:

```toml
routes = [
  { pattern = "wasm.localpdf.org/*", zone_name = "localpdf.org" }
]
```
