...
Host LocalPDF on your own server using Nginx.
...
Create `/etc/nginx/sites-available/localpdf`:
...
```bash
sudo ln -s /etc/nginx/sites-available/localpdf /etc/nginx/sites-enabled/
sudo nginx -t
