...
LocalPDF can be self-hosted on your own infrastructure. This guide covers various deployment options.

## Quick Start with Docker / Podman

The fastest way to self-host LocalPDF:
...
>
> Use `http://localhost` only for same-device testing. If users access LocalPDF through a LAN IP or hostname, terminate it with HTTPS.
...
# Podman
podman run -d -p 3000:8080 ghcr.io/alam00000/localpdf-simple:latest
```
...
Run LocalPDF as a systemd service. Create `~/.config/containers/systemd/localpdf.container`:
...
The Self-Hosted build (the `localpdf-simple` image, also called Simple Mode) is **functionally identical** to the Commercial build — every PDF tool is present and behaves the same. It just hides the marketing chrome that only makes sense on localpdf.org itself or on a commercial public-facing deployment. **It is not a feature-reduced or "lite" version.**
...
The Commercial build (`ghcr.io/alam00000/localpdf:latest`) is what powers localpdf.org itself and is used by commercial license holders running public-facing deployments — it adds the hero, FAQ, testimonials, and footer that wouldn't make sense on an internal tool.

If you're self-hosting LocalPDF for your team, organization, or as an internal tool, pull the Self-Hosted build:

```bash
# Use the pre-built image (recommended)
docker run -p 3000:8080 ghcr.io/alam00000/localpdf-simple:latest

# Or build it yourself
SIMPLE_MODE=true npm run build
```

See [SIMPLE_MODE.md](https://github.com/jacksnotes/LocalPDF/blob/main/SIMPLE_MODE.md) for full details.

### Commercial build (public-facing deployments with your own brand)

The Commercial build (the `localpdf` image — no `-simple` suffix) is what powers localpdf.org itself. It includes the full marketing site (hero, features, FAQ, testimonials, footer) on top of every PDF tool. Use this build when you want to **deploy LocalPDF as a public-facing PDF service under your own brand** — for example:

- You're running LocalPDF as a hosted SaaS for end-users on your own domain
- You want the landing-page experience (marketing sections + tools), not just the bare tool surface
- You're a commercial license holder embedding LocalPDF into a commercial product

**Run it as-is** (LocalPDF branding — useful to preview the build):

```bash
docker run -p 3000:8080 ghcr.io/alam00000/localpdf:latest
```
...
::: warning Licensing
Running the Commercial build is allowed under either of LocalPDF's two license options:
...
See the [Licensing page](https://localpdf.org/licensing.html) for the full comparison. AGPL-licensed WASM modules (PyMuPDF, Ghostscript, CoherentPDF) load from a CDN at runtime, so they don't enter your image and don't change your licensing posture.
:::
...
Replace the default LocalPDF logo, name, and footer text with your own at build time:
...
**Verify**: open DevTools Console on any LocalPDF page and run:
...
LocalPDF **does not bundle** AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box — no manual setup needed.
...
If you need to serve AGPL WASM files with proper CORS headers, you can deploy a simple proxy. See the [Cloudflare WASM Proxy guide](https://github.com/jacksnotes/LocalPDF/blob/main/cloudflare/WASM-PROXY.md) for an example implementation.

::: tip Why Separate?
This separation ensures:

- Clear legal compliance for commercial users
- LocalPDF's core remains under its dual-license (AGPL-3.0 / Commercial)
...
::: tip
LocalPDF is a static site—there's no database or backend server required!
:::
