...
The easiest way to self-host LocalPDF in a production environment.
...
>
> - **Self-Hosted build** — `ghcr.io/alam00000/localpdf-simple:latest`. Every PDF tool, **without** the marketing chrome (no hero, FAQ, testimonials, footer). Use this for internal/team/organization deployments. It is **not** a feature-reduced "lite" version.
> - **Commercial build** — `ghcr.io/alam00000/localpdf:latest`. The full marketing site, used by localpdf.org itself and by commercial license holders running public-facing deployments.
>
...
docker run -d \
  ...
  ghcr.io/alam00000/localpdf-simple:latest
...
services:
  localpdf:
    image: ghcr.io/alam00000/localpdf-simple:latest
...
```bash
docker build -t localpdf:custom .
docker run -d -p 3000:8080 localpdf:custom
```
...
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `SIMPLE_MODE`                        | Build the Self-Hosted variant (hides localpdf.org marketing chrome — hero, FAQ, testimonials, footer). All PDF tools remain. | `false`                                                        |
| `BASE_URL`                           | Deploy to subdirectory                                                                                                       | `/`                                                            |
| `VITE_WASM_PYMUPDF_URL`              | PyMuPDF WASM module URL                                                                                                      | `https://cdn.jsdelivr.net/npm/@localpdf/pymupdf-wasm@0.11.16/` |
| `VITE_WASM_GS_URL`                   | Ghostscript WASM module URL                                                                                                  | `https://cdn.jsdelivr.net/npm/@localpdf/gs-wasm@0.1.1/assets/` |
...
| `VITE_BRAND_NAME`                    | Custom brand name                                                                                                            | `LocalPDF`                                                     |
...
- Air-gapped deployments that override all three `VITE_WASM_*_URL` values also get the public `cdn.jsdelivr.net` removed from CSP (each default is replaced, not appended). Similarly, setting `VITE_CORS_PROXY_URL` replaces the public `localpdf-cors-proxy.localpdf.workers.dev` default.
...
docker run -d -p 3000:8080 localpdf
```

### Custom Branding

Replace the default LocalPDF logo, name, and footer text with your own. Place your logo file in the `public/` folder (or use an existing image), then pass the branding variables at build time:
...
Tool IDs are the page URL without `.html`. For example, if the tool lives at `localpdf.org/edit-pdf.html`, the ID is `edit-pdf`.

#### Finding Tool IDs

The easiest way: open any tool in LocalPDF and look at the URL. The last part of the path (without `.html`) is the tool ID.
...
npm pack @localpdf/pymupdf-wasm@0.11.14
...
# 5. Transfer localpdf.tar + the .tgz packages + tesseract-langdata/ + ocr-fonts/ into the air-gapped network
...
tar xzf localpdf-pymupdf-wasm-0.11.14.tgz -C /var/www/wasm/pymupdf --strip-components=1
tar xzf localpdf-gs-wasm-*.tgz -C /var/www/wasm/gs --strip-components=1
...
By default, LocalPDF listens on port `8080` inside the container. To change this, set the `PORT` environment variable at runtime:

```bash
docker run -p 3000:9090 -e PORT=9090 ghcr.io/alam00000/localpdf-simple:latest
```
...
For environments that require running as a specific non-root user (NAS devices, Kubernetes with security contexts, organizational policies), LocalPDF provides a separate Dockerfile with LSIO-style PUID/PGID support.
...
services:
  ...
  localpdf:
    ...
    labels:
      - 'traefik.enable=true'
      - 'traefik.http.routers.localpdf.rule=Host(`pdf.example.com`)'
      - 'traefik.http.routers.localpdf.entrypoints=websecure'
      - 'traefik.http.routers.localpdf.tls.certresolver=letsencrypt'
      - 'traefik.http.services.localpdf.loadbalancer.server.port=8080'
      # Required headers for SharedArrayBuffer (LibreOffice WASM)
      - 'traefik.http.routers.localpdf.middlewares=localpdf-headers'
      - 'traefik.http.middlewares.localpdf-headers.headers.customresponseheaders.Cross-Origin-Opener-Policy=same-origin'
      - 'traefik.http.middlewares.localpdf-headers.headers.customresponseheaders.Cross-Origin-Embedder-Policy=require-corp'
    restart: unless-stopped
```
...
Create a container unit file at `~/.config/containers/systemd/localpdf.container` (user) or `/etc/containers/systemd/localpdf.container` (system):
...
# Start the service
systemctl --user start localpdf

# Enable on boot
systemctl --user enable localpdf

# Check status
systemctl --user status localpdf

# View logs
journalctl --user -u localpdf -f
```
...
```ini
[Unit]
Description=LocalPDF with health monitoring
After=network-online.target
Wants=network-online.target
...
For custom network configuration, create a network file `localpdf.network`:
